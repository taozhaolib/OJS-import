...
*/
	...
	Class OJSImport extends AbstractPHPExcel 
	{
		...
		private function AddFrontInfo($dom, $issue,$filepath)
		{
			//$frontInfo = $value["FRO"];
		...
		private function addArticleAuthorInfo($dom, $article, $authInfo)
		{
			$articlePriAuthor = $dom->createElement("author");
			$child = $article->appendChild($articlePriAuthor);
			$child->setAttributeNode(new DOMAttr('primary_contact','true'));
			...
			$email = $dom->createElement("email");
			$emailText = $dom->createTextNode($authInfo[Author1_Email]);
			$email->appendChild($emailText);
			$articlePriAuthor->appendChild($email);
		...
		function getXmlFromArray($dtd)
		{
			...
			if(isset($cleanData) && count($cleanData) > 0)
			{
				...
				foreach ($cleanData as $key => $value) 
				{
					...
					*/
					$this->AddFrontInfo($dom, $issue,"/".$key."/".$value["FRO"][Filename]);
					...
					foreach ($value["ART"] as $index => $articleInfo) 
					{
						...
						$this->addArticleAuthorInfo($dom, $article, $articleInfo);
					}
				}
				...
				switch($value[Type]){
					...
					case "ART":
						if($value[Author1_First] != "" && $value[Author1_Last] != "")
							$cleanData[$issue]["ART"][] = $value;
						break;
					case "BRE":
						if($value[Author1_First] != "" && $value[Author1_Last] != "")
							$cleanData[$issue]["BRE"][] = $value;
						break;
					case "MISC":
